// The following has been modified to handle composite token values
// Original: https://github.com/amzn/style-dictionary/blob/main/lib/common/formatHelpers/createPropertyFormatter.js
 ...
 * @param {Object} options.formatting - Custom formatting properties that define parts of a declaration line in code. The configurable strings are: prefix, indentation, separator, suffix, and commentStyle. Those are used to generate a line like this: `${indentation}${prefix}${token.name}${separator} ${token.value}${suffix}`
...
function createPropertyFormatter({
  ...
  return function (token) {
    ...
    to_ret_prop += token.attributes.category === 'asset' ? `"${value}"` : value

    const themeable_prop =
    ...
    if (token.comment && commentStyle !== 'none') {
      if (commentStyle === 'short') {
        to_ret_prop = to_ret_prop.concat(` // ${token.comment}`)
      } else {
        to_ret_prop = to_ret_prop.concat(` /* ${token.comment} */`)
      }
    }
